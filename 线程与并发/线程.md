...
```
threadStatus 初始化为 0.
1. 检查线程状态(threadStatus是否等于 0), 只有 new 状态下的线程才能启动(threadStatus != 0), 否则会抛出`IllegalThreadStateException`异常.
2. 加入线程组.
...
```java
    ...
    所以在直接调用 run 方法的时候, 就没有通过创建的线程,而是直接通过主线程来调用的.
```
...
`Guava`是对Java Future 异步回调的增强. 使用`Guava`异步回调, 也需要用到java的`Callable`接口. 简单的说, 只有在java的`Callable`任务的执行结果出来后, 才能执行到`Guava`中的`FutureCallback`的结果回调.
...
但是, 准确的讲, ==创建线程只有一种方式==, 那就是==构建 Thread 类==,只不过实现线程的执行单元有两种方式:
...
因为 start() 才是真正意义上启动线程的方法, 并且还会检查线程的状态等.
