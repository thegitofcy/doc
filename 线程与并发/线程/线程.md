...
sleep 遇到 interrupt, 就会抛出异常. 并且会清除 interrupt 中断标志.
...
//        stopByInterruptWithSleep();
    ...
    public static void stopByInterruptWithSleep() throws InterruptedException {
        Thread thread = new Thread(() -> {
            int num = 0;
          	// 其实 sleep 遇到 interrupt 都会抛出异常, 所以这里不用判断是否打标
            //while (num < Integer.MAX_VALUE / 2 && !Thread.currentThread().isInterrupted()) {
    ...
    public static void stopByInterruptWithSleepEveryLoop() throws InterruptedException {
        Thread thread = new Thread(() -> {
            int num = 0;
            try {	// try/catch 要放在 while 外边, 如果放 while 里边, 会导致停止失效. 因为 sleep 会清除掉 interrupt 标志.
              // 其实 sleep 遇到 interrupt 都会抛出异常, 所以这里不用判断是否打标
              //while (num < Integer.MAX_VALUE / 2 && !Thread.currentThread().isInterrupted()) {
                while (num < Integer.MAX_VALUE / 2) {
                    if (num % 10000 == 0) {
...
- 发布: 发布是指一个对象被声明为 `public`的, 或者是一个方法的参数, 或者一个方法 return 一个对象, 那么这个对象就是被发布出去了.
- 逸出:
  - 方法返回一个 `private`对象(private 的本意是不让外部访问).
  - 还未完成初始化(构造函数没有完全执行完毕)就把对象提供给外界.比如:
    - 在构造函数中未完成初始化完毕就 this 赋值
    - 隐式逸出-注册监听事件.
    - 构造函数中运行线程.
...
### 2. 实现 Runnable 接口和继承 Thread类那种方式更好?



### 3. 一个线程两次调用 start() 方法会出现什么情况? 为什么



### 4. 既然 start() 方法会调动 run() 方法, 为什么我们选择调用 start()方法,而不是直接调用 run() 方法?
...
### 8. 为什么 wait()需要在同步代码块内使用, 而 sleep()不需要.



### 9. 为什么线程通讯方法wait(), nootify(), notifyAll()被定义在 Object 类里, 而 sleep 被定义在 Thread 类里边?



### 10. wait()方法是属于 Object 对象的, 那调用 Thread.wait 会怎么样?



### 11. 如何选择使用 notify() 和 notifyAll() 方法.



### 12. notifyAll之后, 所有的线程都会再次抢夺锁, 如果某贤臣抢夺失败怎么办?



### 13. 用 suspend() 和 resume() 来阻塞线程可以吗? 为什么?



### 14. wait/notify, sleep异同?(方法属于哪个对象? 线程状态怎么切换)



### 15. 在 join 期间, 线程处于哪种线程状态?
...
### 18. run方法是否可以抛出异常? 如果抛出异常, 线程的状态会怎么样?
...
- 比如两个线程 A 和线程 B, A 先持有锁 1, 然后还需要锁 2, 当时 B 持有锁 2, 并且需要锁 1, A 和 B 都没有释放当前持有的锁, 此时就会发生死锁.
...
```java
// 两个线程, 分别持有自己的锁, 并且不释放锁, 然后等待对方的锁, 就造成了死锁.
public class DeadLock implements Runnable {
    ...
    public static void main(String[] args) {
        ...
        // 线程启动, 获取锁 o1, 然后进行睡眠, 使另外一个线程可以顺利获取到锁 o2, 
        // 然后两个线程分别持有自己的锁,并且不释放, 然后都去尝试获取对方的锁, 这样就发生了死锁
